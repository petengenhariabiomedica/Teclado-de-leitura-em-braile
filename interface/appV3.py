...
def serial_sender():
    ...
    while serial_index < len(current_text) and processing:
        char = current_text[serial_index]  # Não precisa de lock aqui, só leitura
        ...
        if char.isalpha():
            try:
                ...
                if ack == "OK":
                    with lock:
                        serial_index += 1
                        print(f"[✔] Caractere '{char}' enviado e confirmado")  # Log adicional
                else:
...
def stream():
    def generate_serial_stream():
        global sse_index
        print("[SSE] Conexão estabelecida com cliente")  # Log de conexão
        while processing or sse_index < len(current_text):
            with lock:
                if sse_index < serial_index:
                    char = current_text[sse_index]
                    print(f"[SSE] Enviando caractere: {char}")  # Log de envio
                    sse_index += 1
                    yield f"data: {char}\n\n"
                elif sse_index >= len(current_text):
                    print("[SSE] Envio completo")  # Log de conclusão
                    yield "data: done\n\n"
                    break
            time.sleep(0.1)  # Aumentei um pouco o sleep para estabilidade
        ...
        if file and file.filename.endswith('.pdf'):
            try:
                ...
                if bk.connect_serial():
                    ...
                    print(f"[INFO] Texto extraído: {texto[:50]}...")  # Log do texto extraído

                    # Arquivo .txt criado para verificar o texto gerado pelo PDF
                    ...
                    print(f"[INFO] Texto preparado para envio: {current_text[:50]}...")  # Log do texto processado
                    ...
                    # Inicia thread para envio serial
